...
datetimeStamp <- function(
  storeIn,
  goalDate = lubridate::now(),
  basepath = "Data/Pulled Data",
  timeZone = "EST"
) {
  ...
  if (!dir.exists(fullpath)) {
    ...
    choice <- menu(
      c("Yes", "No"),
      title = sprintf(
        "Path '%s' doesn't exist, or isn't a valid directory.\nDo you want to make one with this name?",
        fullpath
      )
    )
    ...
    if (choice == 1) {
      ...
      stop(sprintf(
        "Script cannot continue because '%s' doesn't exist or is not a valid directory.",
        fullpath
      ))
    }
  }
  ...
  source <- str_replace(dirListing, ".(parquet|csv|xlsx)", "") %>%
    (\(x) {
      str_replace(x, "[0-9]{4}_[0-9]{2}_[0-9]{2}(_[0-9]{2}_[0-9]{2}|)", "")
    })() %>%
    (\(y) {
      str_replace(y, "_\\b|\\b_", "")
    })()
  ...
  time <- str_extract(
    ...
    (\(x) {
      parse_date_time(x, c("Ymd HM", "Ymd"), tz = timeZone)
    })()
  ...
  relative_reports <- all_dates %>%
    ...
    mutate(
      Delta = round(
        abs(purrr::map_dbl(History, ~ lubridate::int_length(. %--% goalDate))) /
          (60 * 60 * 24),
        1
      )
    ) %>%
    ...
    mutate(
      "Status" = ifelse(History == max(History), "Recent Pull", "Min Delta")
    ) %>%
    mutate(
      "Status" = ifelse(
        Delta == min(Delta) & History == max(History),
        "Both",
        Status
      )
    ) %>%
  ...
  # Report the results.
  result <- list(
    "Record History" = all_dates,
    "Report Relative to Date" = relative_reports
  )
...
storeFile <- function(
  ...
  mostRecent <- suppressMessages(datetimeStamp(
    storeIn = storeIn,
    basepath = basepath
  ))
  ...
  if (any(mostRecent == "No History")) {
    ...
    !any(mostRecent$`Report Relative to Date`$Source %in% sourceName)
  ) {
    message(sprintf(
      "No existing files for the provided source is in directory '%s'.",
      fullpath
    ))
    ...
    proximity <- mostRecent %>%
      ...
      mutate(
        Delta = abs(
          purrr::map_dbl(History, ~ lubridate::int_length(. %--% time_now)) /
            3600
        )
      ) %>%
    ...
    if (proximity != 0) {
      stop(sprintf(
        "A file for '%s' was recorded within the tolerance range.",
        sourceName
      ))
    }
  }
  ...
  if (fileType == "parquet") {
    ...
    message(sprintf(
      "Wrote recently pulled data as csv to '%s'",
      file.path(basepath, storeIn)
    ))
  }
}
...
runIfExpired <- function(
  ...
  if (any(mostRecent == "No History") | length(mostRecent) != 2) {
    ...
    if (nrow(mostRecent) > 1L) {
      ...
      if (length(numMins) > 1) {
        stop(sprintf(
          "'%s' has more than one file with the same date. Please resolve this manually before reruning the function.",
          sourceName
        ))
      } else if (length(numMins) < 1) {
        stop(sprintf(
          "'%s' draws an unknow error detecting the recently saved data. Check the directory before attempting the function again.",
          sourceName
        ))
      }
      ...
      message(sprintf(
        "More than one file for '%s' is detected to be within the tolerance range.",
        sourceName
      ))
      message(paste0(capture.output(mostRecent), collapse = "\n"))
      use <- if (interactive()) {
        readline(
          prompt = "Referring to the table above, please select one file by providing its numerical row-index: "
        )
      } else {
        which.max(mostRecent$History)
      }
    ...
    if (
      returnRecent == TRUE & mostRecent$History %--% now() < hours(tolerance)
    ) {
      message(sprintf(
        "OLD dataset for '%s' is being called to memory. Save query is within the tolerance level.",
        sourceName
      ))
      ...
      returnRecent == FALSE & mostRecent$History %--% now() < hours(tolerance)
    ) {
      warning(sprintf(
        "Request is too close to the most recent archive of '%s'.",
        mostRecent$filePath
      ))
...
rm_recent_null <- function(data, date_col, dictionary) {
  ...
  assertthat::assert_that(any(
    "Date" %in% sapply(data[, colnames(data) %in% date_col], class)
  ))
  ...
  dates_na_null <- data %>%
    ...
    (\(x) {
      split(x, cumsum(c(TRUE, diff(x) != 7)))
    })()
  ...
  if (length(eval_recent_dates) < 1) {
    stop(sprintf(
      "Most recent entries where %s have NULL or NA's does not include \nthe most recent date. Step not needed; abort operation.",
      str_flatten(variables, collapse = " and ")
    ))
  }
